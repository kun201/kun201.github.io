...
&emsp;&emsp;如果你编辑的是上面所说的文件，那么下面的就是正确答案及步骤：
    ...
    12.【:w man.test.config】，你会发现最下方的状态栏显示“man.test.config" [NEW]..的字样。
    13.【25G】之后，再按下【15x】即可删除15个字符，出现【tree】字样。
    14.先【1G】到第一行，然后按下大写的【O】会在上面新增一行且进入编辑模式；接着你要输入的字符串，按下【Esc】退出编辑模式。
    15.【:wq】。
